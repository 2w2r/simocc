...
import { Check, CircleAlert, CircleCheck, Info, Plus, Unlink, X } from "lucide-react"
...
import { removeSimbriefPilotId, saveSimbriefPilotId } from "@/components/ops/settings/connections/actions"
import {
    Tooltip,
    TooltipContent,
    TooltipProvider,
...
export function SimbriefForm({ currentPilotId }: SimbriefFormProps) {
    ...
    const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
        ...
        if (!trimmed)
            validationErrors.draftPilotId = "Please enter a Pilot ID."
                        ...
                        <FieldLabel className="text-sm h-8 min-w-fit rounded-lg text-primary">
                            <TooltipProvider>
                                <Tooltip>
                                    ...
                                    <TooltipContent align="start" side="bottom" className="max-w-85 flex flex-col items-start">
                                        Your SimBrief account number — permanently assigned, no re-linking needed if you change anything at SimBrief.
                                        <span>
                                            Find yours here:{" "}
                                            <br />
                                            <a
                            ...
                            <>
                                <div className="flex flex-col gap-1 w-full">
                                    <div className="text-sm h-8 min-w-8 px-2.5 flex items-center overflow-scroll rounded-lg border border-transparent bg-muted dark:bg-input/30">
                                        <span className="w-full text-center whitespace-nowrap">{pilotId ?? "Not connected"}</span>
                                    </div>
                                    {errors.general && (
                                ...
                                <div className="flex flex-col gap-1 w-full">
                                    <Input
                                        ...
                                        className={cn(
                                            ...
                                            (errors.draftPilotId || errors.general) && "border-destructive"
                                        )}
                                        value={draftPilotId}
                                        onChange={(e) => {
